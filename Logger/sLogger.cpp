...
void sLogger::loadEngineInfo(int pid, int* engineType, int* coresCnt, int* sampleLen_, int* predictionLen_, int* featuresCnt_, bool* saveToDB_, bool* saveToFile_, sOraData* dbconn_, int* coreId, int* coreType, int* coreThreadId, int* parentCoresCnt, int** parentCore, int** parentConnType, int* sourceTSCnt_, int* TSfeaturesCnt_, int** feature_, numtype** trMin_, numtype** trMax_) {
	if (source==OraData) safecall(oradb, loadEngineInfo, pid, engineType, coresCnt, sampleLen_, predictionLen_, featuresCnt_, saveToDB_, saveToFile_, dbconn_, coreId, coreType, coreThreadId, parentCoresCnt, parentCore, parentConnType, sourceTSCnt_, TSfeaturesCnt_, feature_, trMin_, trMax_);
...
void sLogger::saveCoreNNImage(int pid, int tid, int epoch, int Wcnt, numtype* W, int Fcnt, numtype* F, numtype* a, numtype* dF) {
	if (saveToDB) safecall(oradb, saveCoreNNImage, pid, tid, epoch, Wcnt, W, Fcnt, F, a, dF);
...
void sLogger::loadCoreNNImage(int pid, int tid, int epoch, int Wcnt, numtype* W, int Fcnt, numtype* F, numtype* a, numtype* dF) {
	if (source==OraData) safecall(oradb, loadCoreNNImage, pid, tid, epoch, Wcnt, W, Fcnt, F, a, dF);
