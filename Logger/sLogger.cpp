...
void sLogger::saveClientInfo(int pid, int sequenceId, int simulationId, int npid, const char* clientName, double startTime, double elapsedSecs, char* simulStartTrain, char* simulStartInfer, char* simulStartValid, bool doTrain, bool doInfer, const char* clientXMLfile_, const char* shapeXMLfile_, const char* actionXMLfile_, const char* engineXMLfile_) {
	if (saveToDB) safecall(oradb, saveClientInfo, pid, sequenceId, simulationId, npid, clientName, startTime, elapsedSecs, simulStartTrain, simulStartInfer, simulStartValid, doTrain, doInfer, clientXMLfile_, shapeXMLfile_, actionXMLfile_, engineXMLfile_);
	if (saveToFile) safecall(filedb, saveClientInfo, pid, simulationId, npid, clientName, startTime, elapsedSecs, simulStartTrain, simulStartInfer, simulStartValid, doTrain, doInfer, clientXMLfile_, shapeXMLfile_, actionXMLfile_, engineXMLfile_);
}
//--
...
void sLogger::saveRun(int pid, int tid, int npid, int ntid, int seqId, numtype mseR, int runStepsCnt, char** posLabel, int featuresCnt_, int WTlevel_, numtype* actualTRS, numtype* predictedTRS, numtype* actualTR, numtype* predictedTR, numtype* actualBASE, numtype* predictedBASE) {
...
void sLogger::saveEngineInfo(int pid, int engineType, int sampleLen_, int predictionLen_, int batchSize_, int WNNdecompLevel_, int WNNwaveletType_) {
...
void sLogger::saveEngineCoreInfo(int pid, int coreId_, int layer_, int tid, int coreType_) {
	if (source==OraData) safecall(oradb, saveEngineCoreInfo, pid, coreId_, layer_, tid, coreType_);
}
...
void sLogger::saveCoreLayout(int pid, int tid, int parentCoresCnt, int* parentCore, int* parentConnType) {
	if (source==OraData) safecall(oradb, saveCoreLayout, pid, tid, parentCoresCnt, parentCore, parentConnType);
}
void sLogger::loadCoreLayout(int pid, int tid, int* parentCoresCnt, int** parentCore, int** parentConnType) {
	if (source==OraData) safecall(oradb, loadCoreLayout, pid, tid, parentCoresCnt, parentCore, parentConnType);
}
