...
void sOraData::getFlatOHLCV2(char* pSymbol, char* pTF, char* date0_, int pDate0Lag, int* stepsCnt, char** oBarTime, numtype* oBarData, char* oBarTime0, numtype* oBaseBar) {
	...
	try {
		...
		while (((ResultSet*)rset)->next()&&i>=0) {
			...
			oBarData[(FXDATA_FEATURESCNT_BASE+FXDATA_FEATURESCNT_IND)*i+FXHIGH] = ((ResultSet*)rset)->getFloat(3); high[i]=(numtype)oBarData[(FXDATA_FEATURESCNT_BASE+FXDATA_FEATURESCNT_IND)*i+FXHIGH];
			oBarData[(FXDATA_FEATURESCNT_BASE+FXDATA_FEATURESCNT_IND)*i+FXLOW] = ((ResultSet*)rset)->getFloat(4); low[i]=(numtype)oBarData[(FXDATA_FEATURESCNT_BASE+FXDATA_FEATURESCNT_IND)*i+FXLOW];
			oBarData[(FXDATA_FEATURESCNT_BASE+FXDATA_FEATURESCNT_IND)*i+FXCLOSE] = ((ResultSet*)rset)->getFloat(5); close[i]=(numtype)oBarData[(FXDATA_FEATURESCNT_BASE+FXDATA_FEATURESCNT_IND)*i+FXCLOSE];
			oBarData[(FXDATA_FEATURESCNT_BASE+FXDATA_FEATURESCNT_IND)*i+FXVOLUME] = ((ResultSet*)rset)->getFloat(6); volume[i]=(numtype)oBarData[(FXDATA_FEATURESCNT_BASE+FXDATA_FEATURESCNT_IND)*i+FXVOLUME];
			//--
		...
		for (int f=0; f<FXDATA_FEATURESCNT_BASE; f++)	oBaseBar[f] = ((ResultSet*)rset)->getFloat(f+2);
		...
		for (i=0; i<(*stepsCnt); i++) {
			...
			if (i>=boll_start[0]) oBarData[(FXDATA_FEATURESCNT_BASE+FXDATA_FEATURESCNT_IND)*i+FXBOLLH]=(numtype)bollh[i-boll_start[0]];
			if (i>=boll_start[0]) oBarData[(FXDATA_FEATURESCNT_BASE+FXDATA_FEATURESCNT_IND)*i+FXBOLLM]=(numtype)bollm[i-boll_start[0]];
			if (i>=boll_start[0]) oBarData[(FXDATA_FEATURESCNT_BASE+FXDATA_FEATURESCNT_IND)*i+FXBOLLL]=(numtype)bolll[i-boll_start[0]];
			if (i>=dema_start[0]) oBarData[(FXDATA_FEATURESCNT_BASE+FXDATA_FEATURESCNT_IND)*i+FXDEMA]=(numtype)dema[i-dema_start[0]];
			if (i>=ma_start[0]) oBarData[(FXDATA_FEATURESCNT_BASE+FXDATA_FEATURESCNT_IND)*i+FXMA]=(numtype)ma[i-ma_start[0]];
			if (i>=mom_start[0]) oBarData[(FXDATA_FEATURESCNT_BASE+FXDATA_FEATURESCNT_IND)*i+FXMOM]=(numtype)mom[i-mom_start[0]];
		}
...
void sOraData::saveEngineData(int pid, int IOshift_, int* dataSourcesCnt_, int** featuresCnt_, int*** feature_, int*** dt_, int* WTlevel_, int* WTtype_) {
	...
	try {
		stmt = ((Connection*)conn)->createStatement("insert into EngineData(ProcessId, IOshift, EngineSide, DataSource, Feature, DT, WTlevel, WTtype) values(:P01, :P02, :P03, :P04, :P05, :P06, :P07, :P08)");
...
void sOraData::loadEngineData(int pid, int* IOshift_, int* dataSourcesCnt_, int** featuresCnt_, int*** feature_, int*** dt_, int* WTlevel_, int* WTtype_) {
	...
	try {
		sprintf_s(sqlS, SQL_MAXLEN, "select IOshift from EngineData where ProcessId = %d ", pid);
		stmt = ((Connection*)conn)->createStatement(sqlS);
		rset = ((Statement*)stmt)->executeQuery();
		int i=0;
		while (((ResultSet*)rset)->next()) {
			(*IOshift_)=((ResultSet*)rset)->getInt(1);
			i++;
		}
		if (i==0) fail("Engine pid %d not found.", pid);
		((Statement*)stmt)->closeResultSet((ResultSet*)rset);
...
void sOraData::saveTradeInfo(int MT4clientPid, int MT4sessionId, int MT4accountId, int MT4enginePid, int iPositionTicket, char* iPositionOpenTime, char* iLastBarT, double iLastBarO, double iLastBarH, double iLastBarL, double iLastBarC, double iLastBarV, double iLastForecastO, double iLastForecastH, double iLastForecastL, double iLastForecastC, double iLastForecastV, char* iCurrBarT, double iForecastO, double iForecastH, double iForecastL, double iForecastC, double iForecastV, int iTradeScenario, int iTradeResult, double iTradeProfit, int iTPhit, int iSLhit) {
	...
	sprintf_s(sqlS, SQL_MAXLEN, "insert into TradeInfo(ClientPid, SessionId, AccountId, TicketId, EnginePid, OpenTime, LastBarT, LastBarO, LastBarH, LastBarL, LastBarC, LastBarV, LastForecastO, LastForecastH, LastForecastL, LastForecastC, LastForecastV, BarT, ForecastO, ForecastH, ForecastL, ForecastC, ForecastV, TradeScenario, TradeResult, TradeProfit, TPhit, SLhit) values(%d, %d, %d, %d, %d, to_date('%s','%s'), to_date('%s','%s'), %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, to_date('%s','%s'), %f, %f, %f, %f, %f, %d, %d, %f, %d, %d)", MT4clientPid, MT4sessionId, MT4accountId, iPositionTicket, MT4enginePid, iPositionOpenTime, DATE_FORMAT, iLastBarT, DATE_FORMAT, iLastBarO, iLastBarH, iLastBarL, iLastBarC, iLastBarV, iLastForecastO, iLastForecastH, iLastForecastL, iLastForecastC, iLastForecastV, iCurrBarT, DATE_FORMAT, iForecastO, iForecastH, iForecastL, iForecastC, iForecastV, iTradeScenario, iTradeResult, iTradeProfit, iTPhit, iSLhit);
	safecall(this, sqlExec, sqlS);
