...
void sNN::BP_scgd() {
	...
	do {
		...
		if (success) {
			...
			Alg->Vcopy(weightsCntTotal, scgd->GdJdW, scgd->dE0);
			//-- get dE1 (dJdW at W+sigma*p)
			Alg->Vscale(weightsCntTotal, scgd->p, sigma, scgd->sigmap);
			Alg->Vadd(weightsCntTotal, W, 1, scgd->sigmap, 1, scgd->newW);
			//=================================
			dEcalc(true, true, false, scgd->newW, scgd->dE1);
			//-- calc s
		...
		//--- 6.1 calc scgd->newW=w+alpha*p , which will also be used in (7)
		Alg->Vscale(weightsCntTotal, scgd->p, alpha, dW);
		Alg->Vadd(weightsCntTotal, W, 1, dW, 1, scgd->newW);

		if (success) Alg->Vcopy(weightsCntTotal, W, scgd->oldW);
		//--- 6.2 E(w)
		e_old = Ecalc(NN, Mx, -1, scgd->oldW);
		//--- 6.3 E(w+dw) is calculated by E_at_w()
		e_new = Ecalc(NN, Mx, -1, scgd->newW);
		...
		if (comp>=0) {
			...
			//-- scgd->TotdW = scgd->TotdW + dW
			Alg->Vadd(weightsCntTotal, scgd->TotdW, 1, dW, 1, scgd->TotdW);
			//-- 7.1 recalc  dJdW
			...
			if (((k+1)%nodesCntTotal)==0) {
				...
				numtype _b;
				Alg->Vnorm(weightsCntTotal, scgd->r, &_b);
				b1 = pow(_b, 2);
				Alg->VdotV(weightsCntTotal, scgd->r, scgd->prev_r, &b2);
				beta = (b1-b2)/mu;
				//-- p = r + beta*p
				Alg->Vadd(weightsCntTotal, scgd->p, beta, scgd->r, 1, scgd->p);
			}
		...
		gotoxy(0, Mx->ScreenPos); printf("\rProcess %6d, Thread %6d, Iteration %6d , success=%s, rnorm=%f , norm_ge=%f", pid, tid, k, (success) ? "TRUE " : "FALSE", scgd->rnorm, Mx->NN.norm_ge);
		//ReleaseMutex(Mx->ScreenMutex);

		//if (DebugParms->SaveInternals>0) SaveCoreData_SCGD(NNLogs, pid, tid, 0, Mx->sampleid, Mx->BPCount, k, Mx->SCGD_progK, delta, mu, alpha, beta, lambda, lambdau, pnorm, rnorm, Mx->NN.norm_ge, Vnorm(weightsCntTotal, dW), comp);
