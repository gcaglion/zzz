...
void sCoreLogger::saveRun(int pid, int tid, int npid, int ntid, int barsCnt, int featuresCnt, int* feature, numtype* actualTRS, numtype* predictedTRS, numtype* actual, numtype* predicted) {
	if (saveToDB) {
		safecall(db, saveRun, pid, tid, npid, ntid, barsCnt, featuresCnt, feature, actualTRS, predictedTRS, actual, predicted);
	}
	if (saveToFile) {
		safecall(file, saveRun, pid, tid, npid, ntid, barsCnt, featuresCnt, feature, actualTRS, predictedTRS, actual, predicted);
	}
}
